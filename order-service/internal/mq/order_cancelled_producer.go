...
type OrderCancelledProducer struct {
	Producer  sarama.AsyncProducer
	RetryChan chan *sarama.ProducerMessage
	topic     string
}

func NewOrderCancelledProducer(producer sarama.AsyncProducer) mq.KafkaProducer {
	pr := &OrderCancelledProducer{
		Producer:  producer,
...
func (p *OrderCancelledProducer) Send(ctx context.Context, event mq.KafkaEvent) error {
...
func (p *OrderCancelledProducer) HandleErrors() {
...
func (p *OrderCancelledProducer) Retry() {
...
func (p *OrderCancelledProducer) UnmarshalMetadata(msg *sarama.ProducerMessage, metadata *mq.KafkaMetadata) {
	if msg.Metadata == nil {
		return
	}
...
func (p *OrderCancelledProducer) Topic() string {
	return p.topic
}
