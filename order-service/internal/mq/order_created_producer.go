...
func NewOrderCreatedProducer(producer sarama.AsyncProducer) mq.KafkaProducer {
	pr := &OrderCreatedProducer{
		Producer:  producer,
...
func (p *OrderCreatedProducer) Send(ctx context.Context, event mq.KafkaEvent) error {
...
func (p *OrderCreatedProducer) HandleErrors() {
...
func (p *OrderCreatedProducer) Retry() {
...
func (p *OrderCreatedProducer) UnmarshalMetadata(msg *sarama.ProducerMessage, metadata *mq.KafkaMetadata) {
	if msg.Metadata == nil {
		return
	}
...
func (p *OrderCreatedProducer) Topic() string {
	return p.topic
}
