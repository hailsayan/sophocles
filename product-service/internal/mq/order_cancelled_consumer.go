...
func NewOrderCancelledConsumer(consumer sarama.ConsumerGroup, dataStore repository.DataStore) mq.KafkaConsumer {
...
func (c *OrderCancelledConsumer) Consume(ctx context.Context) error {
	c.wg.Add(1)
	return c.Consumer.Consume(ctx, []string{c.topic}, c)
}

func (c *OrderCancelledConsumer) Handler() mq.KafkaHandler {
	return func(ctx context.Context, body []byte) error {
		event := new(dto.OrderCancelledEvent)
...
func (c *OrderCancelledConsumer) Topic() string {
	return c.topic
}
...
func (c *OrderCancelledConsumer) Setup(_ sarama.ConsumerGroupSession) error { return nil }

func (c *OrderCancelledConsumer) Cleanup(_ sarama.ConsumerGroupSession) error { return nil }

func (c *OrderCancelledConsumer) ConsumeClaim(sess sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	defer c.wg.Done()
